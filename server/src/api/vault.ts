...
router.get('/:scope/:key', route(async (req, res, next) => {
    ...
    const { encryptionKey } = await fs.readJson(
        environment.cruiser_vault_storage_dir + "/" + secret.id
    );
...
router.post('/:scope/:key', route(async (req, res, next) => {
    ...
    .then(async ([result]) => {
        await fs.mkdirp(environment.cruiser_vault_storage_dir + "/");
        await fs.writeJson(
            environment.cruiser_vault_storage_dir + "/" + result.id,
            { encryptionKey }
        );
...
router.delete('/:scope/:key', route(async (req, res, next) => {
    ...
    await fs.remove(environment.cruiser_vault_storage_dir + "/" + secret.id);

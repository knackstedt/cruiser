    ...
    onReceiveLine(line: LogRecord, runHooks = true) {
        ...
        const injectMatchClasses = (
            ...
            for (let i = 0; i < matches.length; i++) {
                ...
                // Find the first span where it's end is after the match start
                const startSpanIndex = spans.findIndex(s => s['end'] > match.start);
                // Find the first span where it's start is after the match end
                let endSpanIndex = spans.findIndex(s => s['start'] > match.end);
                if (endSpanIndex == -1)
                ...
                // Get all of the spans from the start to the end
                const matchedSpans = spans.slice(startSpanIndex, endSpanIndex);

                const startSpan = matchedSpans[0];
                const middleSpans = matchedSpans.slice(1, -1);
                const endSpan = matchedSpans.at(-1);

                // Now, we need to cut apart the start and end spans,
                // and append the class to all of the middle spans.

                if (!startSpan || !middleSpans || !endSpan)
                    debugger;
                ...
                if (startSpan == endSpan) {
                    const startOuter = {
                        ...
                        text: startSpan.text.slice(0, match.start - startSpan['start'])
                    };
                    const middle = {
                        ...startSpan,
                        start: match.start,
                        end: match.end,
                        css: startSpan.css + cssAttribute,
                        class: customClasses,
                        text: startSpan.text.slice(match.start - startSpan['start'], match.end - endSpan['end'])
                    };
                    const endOuter = {
                        ...endSpan,
                        start: match.end,
                        text: endSpan.text.slice(match.end - endSpan['end'])
                    };

                    spans.splice(spans.indexOf(startSpan), 1, startOuter, middle, endOuter);
                }
                else {
                    const startOuter = {
                        ...startSpan,
                    ...
                    const startInner = {
                        ...
                        start: match.start,
                        class: customClasses,
                        text: startSpan.text.slice(match.start - startSpan['start'])
                    }
                    const endInner = {
                        ...endSpan,
                        end: match.end,
                        class: customClasses,
                        text: endSpan.text.slice(0, match.end - endSpan['start'])
                    };
                    const endOuter = {
                        ...endSpan,
                        start: match.end,
                    ...
                    middleSpans
                        .concat(startInner, endInner)
                        .forEach(m => {
                            m.css += cssAttribute;
                            m['class'] = customClasses;
                        });

                    // Remove the start and end spans and replace with the cut apart ones.
                    spans.splice(spans.indexOf(startSpan), 1, startOuter, startInner);
                    spans.splice(spans.indexOf(endSpan), 1, endInner, endOuter);
                }
            }
        }
        ...
        lines.forEach((ln, i) => {
            ...
            const html = parse(ln).spans.map(s => {
                ...
                s['start'] = index;
                s['end'] = index + s.text.length;
                index += s.text.length;
                return s;
            });

            let lineText = html.map(s => s.text).join('');

            // const boldRanges = parseMatches([...lineText.matchAll(/(__[^_]+?__)/g), ...lineText.matchAll(/(\*\*[^\*]+?\*\*)/g)]);
            // injectMatchClasses(boldRanges, lineText, html, "font-weight: bold");
            // lineText = html.map(s => s.text).join('');

            // const italicRanges = parseMatches([...lineText.matchAll(/(_[^_]+?_)/g), ...lineText.matchAll(/(\*[^\*]+?\*)/g)]);
            // injectMatchClasses(italicRanges, lineText, html, "font-style: italic");
            // lineText = html.map(s => s.text).join('');

            // const strikeRanges = parseMatches([...lineText.matchAll(/(~~[^~]+?~~)/g)]);
            // injectMatchClasses(strikeRanges, lineText, html, "text-decoration: line-through");
            // lineText = html.map(s => s.text).join('');

            const codeRanges = parseMatches([...lineText.matchAll(/(`[^`]+?`)/g)]);
            injectMatchClasses(codeRanges, lineText, html, "", "code");
            lineText = html.map(s => s.text).join('');
            ...
            if (ln.includes("foo"))
                debugger;
            // TODO: Should I entirely rewrite this logic so that it's cohesive with the range detection?
